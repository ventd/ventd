...
- `internal/setup/orchestrator/probe.go` — `ProbePhase` for the v0.8.x orchestrator. Side-effect-free enumeration of controllable PWM channels under `RunContext.HwmonRoot` (production `/sys/class/hwmon`; tests inject a fixture). Produces `ProbeArtifact{Fans []ProbedFan, HwmonRoot}`. Each `ProbedFan` carries `Index`, `PWMPath`, `EnablePath`, paired `RPMPath` (best-effort: pairs `pwmN` with `fanN_input` under the same chip), `ChipName` (read from hwmon `name`), and `LabelHint` (driver-supplied `pwmN_label` when present, else synthesised from chip name). Skips read-only PWMs without a sibling `pwmN_enable` (e.g. `nct6683` loaded for an NCT6687D chip — read-only monitoring values, not controllable). Wired into the env-gated orchestrator preview after `DriverInstall`. Reuses the same `hwmon.FindPWMPathsAt` (newly exported from `internal/hwmon/diagnose.go`) the legacy `Manager.run` path uses, so both paths converge on identical PWM enumeration. Goal 4 surface (*minimal user input on arbitrary hardware*) starts here — combined with the orchestrator's preceding phases, a host with already-controllable PWMs now produces a complete read-only setup-state without any operator interaction. ApplyPhase (which writes `config.yaml`) lands in a subsequent PR alongside Polarity/Calibrate/Verify, because the config schema requires calibration data those phases produce. (PR#B2 of the v0.8.x wizard rework sequence.)
