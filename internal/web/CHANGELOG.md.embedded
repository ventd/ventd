...
- **First-boot daemons can leak `pwm_enable=1` on exit** (tracked as #1312, deferred from v1.0.4). Watchdog registration gap during the polarity auto-probe; masked on dell-smm hardware (SMI-private fan control ignores sysfs `pwm_enable`); real on hosts with direct-control hwmon drivers (NCT6687, IT87, k10temp-based ECs). Fix deferred to a focused follow-up PR with proper hwmon-write-path watchdog registration.
