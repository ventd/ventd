...
<<<<<<< HEAD
=======
>>>>>>> 4600d7b (fix(setup): exclude uncontrollable fans from active config (#598))
...
=======
>>>>>>> fd46ac4 (fix(setup): exclude uncontrollable fans from active config (#598))
- `internal/hwdb/profile_v1.go::ChipProfile.CalibrateWithinChipParallel` — **per-chip-family parallel-safe flag** (#1219). YAML key `calibrate_within_chip_parallel: true` opts a chip family into within-chip-group parallel calibrate sweeps. Default false preserves the pre-#1219 serial-within-chip behaviour; cross-chip parallelism is unaffected either way. Enabled for `nct6687` + `nct6686` (per-channel pwm_enable registers, HIL-verified 2026-05-20 on Phoenix's 13900K + MSI PRO Z690-A DDR4). NCT6798/IT8688/IT8665 stay opt-in pending HIL verification.
- `internal/hwdb/profile_v1.go::IsChipCalibrateWithinChipParallel` — accessor consulted by the orchestrator's `CalibratePhase`. A nil catalog or unknown chip returns false (conservative default).
- `internal/setup/orchestrator/calibrate.go::CalibratePhase.WithinChipParallel` — new injectable predicate field. Production wiring at `internal/setup/orchestrator_bridge.go` passes a closure backed by `hwdb.IsChipCalibrateWithinChipParallel`. With the flag on, an 8-fan-on-one-chip wizard drops from ~5 min to ~1 min of calibrate wall-clock (legacy in-chip serialisation × 8 → 8-way fan-out × ~50 s).
=======
...
>>>>>>> f30629e (feat(hal): msi-ec shift_mode as PowerProfileBackend capability (#1166))
=======
<<<<<<< HEAD
- `internal/calibrate/allow_stop_gate.go::VerifyAllowStopGate` — **safety gate on `allow_stop=true`** (#600, RULE-CAL-ALLOW-STOP-GATED). Daemon startup now refuses to run if any fan has `allow_stop: true` without a stall-calibration record carrying both `StartPWM > 0` (pwm_min_start) and `StopPWM > 0` (pwm_min_running). Hand-edited YAMLs that flip `allow_stop` without first running the wizard get a per-fan refusal with concrete remediation ("Recalibrate before enabling allow_stop"). NVIDIA fans are skipped (NVML's percent floor is non-zero by API contract). Fresh installs with the wizard-default config (`min_pwm=80`, `allow_stop=false`) pass vacuously.
- `cmd/ventd/main.go` — wires the gate into the `LoadForStartup` → daemon-init path, after config validation but before any HAL Write would fire. Non-first-boot only (a fresh-install path can't have AllowStop without a wizard run anyway).

>>>>>>> 67985eb (fix(calibrate): safety gate allow_stop=true on stall calibration (#600))
>>>>>>> b3411f3 (fix(calibrate): safety gate allow_stop=true on stall calibration (#600))
=======
...
- `internal/setup/orchestrator/apply.go::buildConfig` — extended signature: takes a `map[string]bool` of PWMPaths RPMDetect actively cleared, returns `(*config.Config, []UncontrollableFan)`. Exclusion is gated on the explicit "RPMDetect ran AND found no correlation" signal so wizard runs that skip RPMDetect (e.g. tests, monitor-only outcomes) preserve pre-#598 admit-with-empty-RPMPath behaviour.
>>>>>>> 85e2783 (fix(setup): exclude uncontrollable fans from active config (#598))
>>>>>>> fd46ac4 (fix(setup): exclude uncontrollable fans from active config (#598))
>>>>>>> 4600d7b (fix(setup): exclude uncontrollable fans from active config (#598))
