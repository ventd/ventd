...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-01_dimensionless_within_host

## RULE-R33-LOCK-02: Per-fan score is the four-term sum S_tip + S_tone + S_motor + S_pump.

`Score(Fan)` = `Tip(...) + Tone(...) + Motor(...) + Pump(...)`. All four
terms are non-negative; the sum is the per-fan score in au.

Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-02_four_term_sum

## RULE-R33-LOCK-03: Broadband term scales as 50·log10(RPM·D / D_ref·RPM_ref) per Madison/AMCA.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-03_broadband_50log10_scaling

## RULE-R33-LOCK-04: Tonal term sums harmonics k ∈ {1, 2, 3} of BPF = B·RPM/60 with weights {1.0, 0.5, 0.25}.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-04_tonal_harmonic_weights

## RULE-R33-LOCK-05: Tonal masking — at high broadband levels, harmonics fall below M_thr and S_tone collapses.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-05_tonal_masking_threshold

## RULE-R33-LOCK-06: Motor whine ramps down to zero with RPM; broadband-rise above class anchor masks the floor.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-06_motor_decays_with_rpm

## RULE-R33-LOCK-07: Pump-vane-tone band at f = RPM × N_vanes / 60; A-weighting clamped to non-negative; default N_vanes = 6.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-07_pump_vane_tone_band

## RULE-R33-LOCK-08: Multi-fan composition is energetic: S_host = 10·log10(Σ 10^(S_fan/10)).
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-08_compose_energetic_sum

## RULE-R33-LOCK-09: Score evaluation never blocks on I/O.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-09_no_mic_dependency

## RULE-R33-LOCK-11: Default blade counts per class match R33 §2.2 — 7 / 9 / 11 / 27 / 33.

Verified by inspecting the `classes` table directly: case axial = 7,
radiator = 9, GPU shroud = 11, NUC blower = 27, laptop blower = 33,
server high-RPM = 7.

Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-11_default_blade_counts

## RULE-R33-LOCK-12: Wrong blade count produces ≤ 8% score error and preserves within-host ranking.

Five operating points scored with blade=7 vs blade=9 must share an
identical sort order. Verified by `argsort` comparison.

Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-12_blade_count_robustness

## RULE-R33-LOCK-13: Per-tick proxy evaluation is O(N_fans) and ≤ 4 µs/fan budget on commodity CPU.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-13_per_tick_budget

## RULE-R33-LOCK-14: Proxy never opens an audio device, links no audio library, and emits no audio.
...
Bound: internal/acoustic/proxy/proxy_test.go:R33-LOCK-14_no_audio_io
...
Bound: internal/acoustic/proxy/proxy_test.go:CaseFanSlope_MatchesR29Measurement
Bound: internal/acoustic/proxy/proxy_test.go:GPUFanSlope_HigherThanCase
Bound: internal/acoustic/proxy/proxy_test.go:PresetMultiplierOrdering
Bound: internal/acoustic/proxy/proxy_test.go:DefaultRpmPerPWM_Applied
Bound: internal/acoustic/proxy/proxy_test.go:DefaultPreset_AppliedWhenZero
