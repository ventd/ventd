# P2-ASAHI-01 — Apple Silicon (Asahi Linux) backend

**Care level:** LOW-MEDIUM. Read-mostly for now. Writes depend on what the Asahi kernel exposes; currently many M-series machines only surface fan RPM monitoring, not fan PWM write. The backend must gracefully report `hal.CapRead` without `hal.CapWritePWM` on those machines. Work the task however your session is configured — no model-gated abort.

## Task

- **ID:** P2-ASAHI-01
- **Track:** ASAHI (Phase 2)
- **Goal:** Detect Apple Silicon via `/proc/device-tree/compatible` containing `apple,t8103`, `apple,t8112`, `apple,m1`, or related. Reports fan channels via the hwmon layer (Asahi upstream exposes them there), but adds role classification and an Asahi-specific enumeration guard so this backend can ship before full Apple Silicon SMC writes are supported upstream.

## Context

- `internal/hal/backend.go`, `internal/hal/hwmon/backend.go`.
- Asahi Linux hardware support matrix.
...
- Tests outside scope per testplan R19.
...
- `cmd/ventd/main.go` (registration)
- `CHANGELOG.md`
