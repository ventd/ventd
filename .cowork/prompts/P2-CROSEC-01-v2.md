...
**Care level:** HIGH. Laptop EC writes can lock the EC into an unresponsive state requiring a hardware reset. Protocol correctness is non-negotiable. Never write EC commands without first succeeding a HELLO. Work the task however your session is configured — no model-gated abort.
