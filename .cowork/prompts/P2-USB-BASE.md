...
**Model:** Opus 4.7 (CROSEC / LIQUID will both depend on this; correctness in the shared primitive layer compounds).
**Care level:** HIGH. Other Phase 2 backends are blocked on this; if the primitive is wrong, fixes cascade into four backend PRs.
...
- Tests outside the scope this task targets per the testplan catalogue. P-task PRs add tests only as documented in testplan §18 row R19; T-task PRs add only the tests their entry specifies.

## Branch and PR

- Branch: `claude/P2-USB-BASE-primitive-layer`.
- Commit style: conventional commits.
- Open a draft PR with title: `feat(hal/usbbase): shared USB HID primitive layer (P2-USB-BASE)`.
