...
**Care level:** HIGH. IPMI sends raw bytes to a BMC that controls server-grade fan hardware. A malformed command can spin fans to 100% continuously or shut them off. Every write path MUST have a fallback to firmware-auto on daemon exit. Every vendor command must be DMI-gated (chassis_type=23 or vendor string match) so a desktop without IPMI can't accidentally send BMC commands to random `/dev/ipmi*` devices. Work the task however your session is configured — no model-gated abort.
