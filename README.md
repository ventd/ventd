...
* **GPU:** NVIDIA via NVML (temperature reading works out of the box; GPU fan *writes* are enabled by default and need a driver new enough to expose `nvmlDeviceSetFanSpeed_v2` — R515+, Maxwell or newer. The driver additionally gates the write behind a privilege check, so a headless or service-user daemon may see `Insufficient Permissions` until the recommended udev-group rule is applied, see [docs/nvidia-fan-control.md](https://github.com/ventd/ventd/blob/main/docs/nvidia-fan-control.md)); AMD via the amdgpu backend — temperature and fan RPM read out of the box, while fan *writes* require the `--enable-amd-overdrive` experimental flag (it sets `amdgpu.ppfeaturemask`, a kernel taint) and currently cover RDNA1/2 direct-PWM cards; RDNA3+ (the `fan_curve` interface) is monitor-only for now. Intel Arc is read-only at the kernel level.
