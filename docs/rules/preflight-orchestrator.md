...
## RULE-PREFLIGHT-SB-10: MOK enroll AutoFix pipes the password to mokutil twice.

shim's firmware MOK Manager enforces a minimum password length;
empty queues are rejected with "unacceptable password length"
(caught on Phoenix's HIL desktop where firmware refused empty
stdin). mokutil --import reads the password TWICE with echo off
and only queues when both reads match. The AutoFix MUST pipe
`SecureBootProbes.MOKPassword` to mokutil's stdin via two echo
statements so both reads see the same value AND so the operator
can be told the password to type at firmware.

Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-10_enroll_pipes_password_to_mokutil_twice

## RULE-PREFLIGHT-SB-11: Generated MOK password format is "ventd-XXXX" (4 hex chars).

`generateMOKPassword` produces a 10-char string with the literal
prefix "ventd-" and 4 hex chars from crypto/rand. Long enough to
clear shim's minimum-length check (we haven't seen one above 8 in
the wild), short enough to type at the firmware screen where
there's no copy-paste and the keyboard layout may be quirky. The
stable prefix lets operators recognise the password as one
generated by ventd vs random goop.

Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-11_password_format_is_ventd_4hex

## RULE-PREFLIGHT-SB-12: MOK enrollment password is session-cached at /var/lib/ventd/.mok-session-password (mode 0600), reused across re-runs, removed on enrollment.

Operators who run `ventd preflight`, type the wrong password at the
firmware MOK Manager screen, and re-run preflight MUST see the
SAME password — fresh-generation each invocation would force them
to learn a new password they didn't write down.

`loadOrGenerateMOKPassword(path)` is the cache layer:
- Reads existing password from `path` when present, mode ≤ 0600.
- Rejects an over-permissive (≥ 0644) cache with a noisy error —
  operator-edited password files are out of scope.
- Treats a whitespace-only file as missing and regenerates.
- On miss: generates fresh + writes atomically (tmpfile → rename)
  with mode 0600.
- Empty path → no caching (test + legacy invocation seam).

`makeMOKEnrolledWithCleanup(path, underlying)` removes the cache
file when enrollment is observed. The password is no longer
needed post-enrollment; leaving it on disk extends the leak
window for a secret that's served its purpose.
`liveMOKEnrolledWithCleanup(path)` wires the production
underlying via `liveMOKEnrolled`; tests pass a stub directly.

Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_mok_password_is_session_cached
Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_empty_path_disables_caching
Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_overpermissive_cache_is_rejected
Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_empty_cache_file_regenerates
Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_cleanup_removes_cache_on_enrollment
Bound: internal/preflight/checks/secure_boot_test.go:RULE-PREFLIGHT-SB-12_cleanup_keeps_cache_when_not_enrolled
