# Changelog — v0.4.x and earlier

Historical release notes for ventd releases predating v0.5.0. Archived
here in 2026-05-17 so the live `CHANGELOG.md` only carries the current
release line (v0.5.x onward) and stays under a reasonable size for
operators reviewing recent changes.

The format and version-tag conventions are unchanged from the live
file; entries are presented newest-first by date.
...
### Fixed
- Install-contract drift fixes (spec-06 PR 1) (#625)
...
### Dead
- Prune 6 unreachable functions (closes #268, ultrareview-1) (#278)
...
### Validation
- Spare-VM UFW dry-run harness (#99)
...
### Safety
- Shutdown-clean sessions, panic-safe watchdog, mode-guarded PWM- Consolidate JSON encoder logging and drain errCh on shutdown (#3)
