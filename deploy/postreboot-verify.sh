#!/usr/bin/env bash
# deploy/postreboot-verify.sh — runs the 4a–4i reboot-survival assertions
# after a fresh boot, to prove that the hwmonN → chip_name resolution
# survives kernel renumbering and the daemon starts clean from a cold
# cache.
#
# Shipped to /usr/local/sbin/ventd-postreboot-verify.sh. Invoked from
# deploy/ventd-postreboot-verify.service (systemd Type=oneshot,
# After=ventd.service, WantedBy=multi-user.target). Installed opt-in by
# scripts/install.sh and scripts/postinstall.sh when
# VENTD_INSTALL_POSTREBOOT_VERIFY=1.
#
...
# which sources validation/postreboot-verify.sh directly. A BASH_SOURCE
# guard at the bottom keeps `source`'ing the file side-effect-free;
# only direct invocation runs the full matrix.
