...
func TestRegression_Issue466_FirstBootReloadStartsControllers(t *testing.T) {
	...
	go func() {
		done <- runDaemonInternal(ctx, bootCfg, cfgPath, "", logger, "", nil, restartCh, experimental.Flags{}, nil, nil)
	}()
...
func TestRegression_Issue466_NoSelfRestart(t *testing.T) {
	...
	go func() {
		done <- runDaemonInternal(ctx, cfg, cfgPath, "", logger, "", nil, restartCh, experimental.Flags{}, nil, nil)
	}()
