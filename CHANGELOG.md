...
- `internal/calibrate/calibrate.go` + `internal/calibrate/identity.go` + `cmd/ventd/main.go` + `cmd/ventd/runsetup.go` — calibration storage moved from `/etc/ventd/calibration.json` to `/var/lib/ventd/setup/calibration.json` and the on-disk schema bumped to v3. v3 envelopes carry a `hardware_identity` field: a 16-char hex hash of `DMI board_vendor + board_name + sorted hwmon chip names`. On load, a mismatch against the live host's identity drops every record and emits a `calibration.hardware_changed` diagnostic (autofix → `/api/setup/start`). v1 and v2 envelopes migrate transparently: the loader stamps the current identity on first read so a binary upgrade alone does NOT lose calibration. `calibrate.MigrateLegacyPath` relocates any pre-v0.8.x `/etc/ventd/calibration.json` to the new location on the first daemon start of an upgraded host (atomic cross-fs copy, tombstone left at the old location for one release cycle). `internal/web/server.go::handleSetupReset` + `handleFactoryReset` now `rm -rf /var/lib/ventd/setup/` in addition to the existing config + KV wipes — one canonical wipe target for the v0.8.x rework's "stale state never carries into a fresh wizard" goal. First foundational PR for the v0.8.x setup-wizard rework (Goal 3). Goal 3.
